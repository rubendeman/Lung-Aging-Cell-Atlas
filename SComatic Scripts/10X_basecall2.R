...
## eg: Rscript 10X_basecall2.R /gpfs/gibbs/pi/kaminski/public/Backup/Ruben/BaylorFASTQoutput
#####################################################################################################################################################
...
jobsub.filepath <- file.path(basecall.scripts.dir,"basecall2.jobsub.bat")
...
for(i in 1:length(soup.sample.names)){
        script.filepath <- file.path(basecall.scripts.dir, paste(soup.sample.names[i], "basecall2.sh", sep = ""))
##### print the script ##  -N ",num.reads.per.cell,";
        ...
        cmd.out <- paste(cmd.out,"#SBATCH --time=8:00:00 -p day --ntasks=1 --cpus-per-task=8",
          " --mem=60G --job-name=", paste0(soup.sample.names[i], "basecall"),
          " -o ",script.filepath,".o%J -e ",script.filepath,".e%J\n",sep="")
        cmd.out <- paste(cmd.out,
        "mkdir ", soup.sample.dir.paths[i], "/basecall2\n",
"module load miniconda\n",
"conda activate SComatic\n",sep="")
                    cmd.out <- paste(cmd.out, "python ", py.path2, 
                    " --infile ", soup.sample.dir.paths[i], "/basecount/BaseCellCounts.AllCellTypes.tsv",
