...
## eg: Rscript 10X_basecall1.R /gpfs/gibbs/pi/kaminski/public/Backup/Ruben/BaylorFASTQoutput
#####################################################################################################################################################
...
for(i in 1:length(soup.sample.names)){
        script.filepath <- file.path(basecall.scripts.dir, paste(soup.sample.names[i], "basecall1.sh", sep = ""))
...
"conda activate SComatic\n",sep="")
        cmd.out <- paste(cmd.out, "python ", py.path1, 
                    " --tsv_folder ", soup.sample.dir.paths[i], "/basecount/",
                    " --outfile ", soup.sample.dir.paths[i], "/basecount/BaseCellCounts.AllCellTypes.tsv\n",sep="")
