...
cellt <- soup.subset@meta.data$predicted.id
...
res.t <- apply(count_mat, 1, function(gene){
  tmp.df <- data.frame(y=gene, norm_sf=sf, mut=as.numeric(mut), type=as.numeric(factor(cellt)),sub=as.numeric(factor(subject)))
  #f2 <- try(glmer(y ~ age + offset(log(norm_sf)) + (1 | sub), data = tmp.df, family = poisson))
  f2 <- try(glmmTMB(y ~ mut + offset(log(norm_sf)) + (1 | type) + (1 | sub), data = tmp.df, family = nbinom2)) #zi = ~ age;
    res2 <-try(c(
...
outputTable.file <- file.path(my.workingDir, paste0("age-glmmTMB_global_resultsTable.txt"))
